...
import { upsertMehrarbeit, deleteMehrarbeit as deleteMehrarbeitQuery, getHaushaltsjahrById } from "@/lib/db/queries";
...
export async function saveMehrarbeit(formData: FormData) {
  ...
  // Sperre pruefen
  const hjId = safeFormNumber(formData, "haushaltsjahrId");
  if (Number.isFinite(hjId) && hjId > 0) {
    const hj = await getHaushaltsjahrById(hjId);
    if (hj?.gesperrt) {
