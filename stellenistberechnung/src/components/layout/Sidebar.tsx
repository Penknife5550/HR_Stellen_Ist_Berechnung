...
interface NavSection {
  /** Sektions-Ueberschrift (null = keine Ueberschrift, z.B. Dashboard) */
  label: string | null;
  items: NavItem[];
}
 ...
 *
 * 1. STELLENSOLL: Eingaben (Schuelerzahlen, SLR, Zuschlaege) → Berechnung
 * 2. STELLENIST:  Eingaben (Deputate, Mehrarbeit) → Berechnung
 * 3. ERGEBNIS:    Vergleich → Export → Historie
 * 4. VERWALTUNG:  Einstellungen + Admin
 */
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/pflichtstunden", label: "Pflichtstunden", icon: "BookOpen" },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/mehrarbeit", label: "Mehrarbeit", icon: "CirclePlus" },
...
/** Admin-Items werden in die Verwaltungs-Sektion eingefuegt (Rollen-gefiltert) */
...
export function Sidebar({ userName, userRolle }: SidebarProps) {
  ...
  /** Rendert einen einzelnen Nav-Link */
  ...
  return (
    <aside className="no-print fixed left-0 top-0 h-screen w-[280px] bg-[#575756] text-white flex flex-col overflow-y-auto">
      ...
      {/* Navigation — Prozessorientiert gruppiert */}
      <nav className="flex-1 py-2">
        {NAV_SECTIONS.map((section, idx) => (
          <div key={section.label ?? "top"}>
            {/* Trenner zwischen Sektionen (nicht vor der ersten) */}
            {idx > 0 && section.label && (
            ...
            {/* Nav-Items der Sektion */}
            <ul className="space-y-0.5">
              {section.items.map(renderNavItem)}

              {/* Admin-Items in der Verwaltungs-Sektion einfuegen */}
              {section.label === "Verwaltung" && visibleAdminItems.map(renderNavItem)}
            </ul>
          </div>
        ))}
      </nav>
