...
// ============================================================
// PFLICHTSTUNDEN (Vollzeitdeputat je Schulform)
// ============================================================

export async function getAllPflichtstunden() {
  return db.select().from(pflichtstunden).where(eq(pflichtstunden.aktiv, true)).orderBy(asc(pflichtstunden.id));
}

export async function getPflichtstundenBySchulform(schulformName: string) {
  const [result] = await db
    .select()
    .from(pflichtstunden)
    .where(and(eq(pflichtstunden.schulform, schulformName), eq(pflichtstunden.aktiv, true)));
  return result ?? null;
}
...
export async function getSchuljahrByBezeichnung(bezeichnung: string) {
  const [result] = await db
    .select()
    .from(schuljahre)
    .where(eq(schuljahre.bezeichnung, bezeichnung));
  return result ?? null;
}
...
export async function updateSchuljahrAktiv(id: number, aktiv: boolean) {
  // Transaktion: Wenn aktiv=true gesetzt wird → alle anderen auf false setzen (nur ein aktives SJ)
  // Ohne Transaktion koennte eine Race-Condition dazu fuehren, dass 0 oder 2 SJ aktiv sind.
  return db.transaction(async (tx) => {
    if (aktiv) {
      await tx.update(schuljahre).set({ aktiv: false });
    }
